...
test('STAGE 11.35 — Dedicated Non-Production Attachment Upload Concurrency & Load Suite', async (t) => {
  ...
  await t.test('Concurrent Upload Benchmark with Mixed Valid, Spoofed, Oversize & Unauthorized Payloads', async () => {
    ...
    const testJobs = [
      ...
      { id: 'JOB-4', name: 'large_catalog.pdf', mime: 'application/pdf', buffer: createPdfBuffer(2 * 1024 * 1024), auth: { role: 'OWNER', organisationId: 'ORG-ZAMORIN', assignedCafeIds: [] }, cafeId: 'CAFE-01', orgId: 'ORG-ZAMORIN', expectSuccess: true },
