...
- [x] **Password Security (NIST SP 800-63B-4 & Memory-Hard scrypt KDF)**: Minimum 15 characters (single-factor) / 8 characters (MFA-enforced), max 128 characters without truncation via canonical memory-hard scrypt KDF (`$scrypt$v=1$`), legacy `$2b$` and intermediate `$v2$` compatibility with transparent on-login migration, passphrases and Unicode NFC normalization supported, offline blocklist active, 0 mandatory composition rules, 0 arbitrary expiration, zero password-shucking risk.
