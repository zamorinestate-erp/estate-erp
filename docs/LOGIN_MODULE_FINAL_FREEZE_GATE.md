...
- [x] **Password Security (NIST SP 800-63B-4 & 72-Byte Safe Bcrypt)**: Minimum 15 characters (single-factor) / 8 characters (MFA-enforced), max 128 characters without truncation via versioned SHA-256 pre-hashed bcrypt (`$v2$`), legacy `$2b$` compatibility, passphrases with spaces and Unicode supported, offline blocklist active, 0 mandatory composition rules, 0 arbitrary expiration.
