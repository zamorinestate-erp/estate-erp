...
|---|---|---|---|
| **Password Storage** | Versioned Pre-Hashed Bcrypt (`$v2$`) | `SHA-256` pre-hash + `bcrypt` (Cost 12), CSPRNG 128-bit salt | Eliminates 72-byte truncation; supports 128 chars and Unicode |
...
1. **AAL2 (Authenticator Assurance Level 2)**:
   - **ESTABLISHED**: Multi-factor authentication combining memorize-secret (password/passphrase) and out-of-band/software cryptographic authenticator (RFC 6238 TOTP) with replay resistance and rate limiting.
